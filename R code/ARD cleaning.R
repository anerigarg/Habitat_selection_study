...
# get 0 values for damselfish - 0-3 ---------------------------------------
...
ARD_3_dam <- ARD %>% 
  filter(size.class == "0-3") %>% 
  filter(Genus %in% c("Stegastes", "Microspathodon"))

# group by date instead, that way you can do the cut-off to get rid of the first day and get rid of the day where the boat was driving over us
ARD_3_dam2 <- ARD_3_dam %>% 
  dplyr:: group_by(plot, grid_number, date) %>% 
  dplyr:: summarise(abundance = n()) %>% 
  dplyr:: ungroup() %>% 
  complete(nesting(plot, grid_number), date, fill = list(abundance = 0))
...
ARD_3_dam2$plot_grid <- paste(ARD_3_dam2$plot, "-", ARD_3_dam2$grid_number)
ARD_lookup <- read_csv("data/ARD_lookup.csv") # add treatment and complexity:
ARD_3_dam3 <- full_join(ARD_3_dam2, ARD_lookup)
...
ARD_3_dam3$plot_date <- paste(ARD_3_dam3$plot, "-", ARD_3_dam3$date)
ArtR_days_since_lookup$plot_date <- paste(ArtR_days_since_lookup$plot, "-", ArtR_days_since_lookup$date)
...
ARD_3_dam4 <- inner_join(ARD_3_dam3, ArtR_days_since_lookup)
hist(ARD_3_abun4$abundance) # well that's a poisson/neg binom <- consider making a cut off at 20 to reduce outliers
range(ARD_3_abun4$abundance)
...
# get species richness values (ARD6) ---------------------------------------------
...
unique(ARD$common_name) #about 43 spp over whole exp (from 0-6cm)

# combine plot_grid: 
ARD_6_rich$plot_grid <- paste(ARD_6_rich$plot, "-", ARD_6_rich$grid_number)
ARD_lookup <- read_csv("data/ARD_lookup.csv") # add treatment and complexity:
ARD_6_rich1 <- full_join(ARD_6_rich, ARD_lookup)
...
ARD_6_rich1$plot_date <- paste(ARD_6_rich1$plot, "-", ARD_6_rich1$date)
ArtR_days_since_lookup$plot_date <- paste(ArtR_days_since_lookup$plot, "-", ArtR_days_since_lookup$date)
...
ARD_6_rich2 <- inner_join(ARD_6_rich1, ArtR_days_since_lookup)
hist(ARD_6_rich2$rich) # well that's a poisson/neg binom <- consider making a cut off at 20 to reduce outliers
range(ARD_6_rich2$rich) # 0-8

# remember that day 4 was when that boat was driving over us all day --> exclude from analysis
ARD_6_rich3 <- ARD_6_rich2 %>% 
  filter(days_since_outplanting != 4)

### USE THIS DF FROM NOW ON ###
write.csv(ARD_6_rich3, "ARD_6_rich.csv")

unique(ARD_6_rich3$plot)
unique(ARD_6_rich3$days_since_outplanting)

#quick visuals for fun

ARD_6_rich_sum <- ARD_6_rich3 %>% 
  na.omit() %>% 
  group_by(treatment, complexity) %>% 
  summarize(mean.rich = mean(rich))
...
# 4.1) calculate relative richness (0-6) ----------------------------------
...
#1) 1 df with everything but control
ARD_6_rich_no_control <- ARD_6_rich %>% 
  filter(treatment != "control")

#2) add col called days_comp
ARD_6_rich_no_control$days_comp <- paste(ARD_6_rich_no_control$days_since_outplanting, "-", ARD_6_rich_no_control$complexity)

#3) 1 df with just control
ARD_6_rich_control <- ARD_6_rich %>% 
  filter(treatment == "control")

#4) add col days_comp
ARD_6_rich_control$days_comp <- paste(ARD_6_rich_control$days_since_outplanting, "-", ARD_6_rich_control$complexity)

#5) make a lookup that has just the mean rich for a certain day and complexity
ARD_6_rich_control_lookup <- ARD_6_rich_control %>% 
  group_by(days_comp) %>% 
  summarize(c.rich.mean = mean(rich))

#join the the lookup to the df with no control plots
ARD_6_rich_joined <- full_join(ARD_6_rich_no_control, ARD_6_rich_control_lookup)

# calculate relative rich (for a given day and complexitym that rich - the background rich for that day)
ARD_6_relrich <- ARD_6_rich_joined %>% 
  mutate(rrich = rich - c.rich.mean)

write.csv(ARD_6_relrich, "ARD_6_relrich.csv") 

hist(ARD_6_relrich$rrich)
